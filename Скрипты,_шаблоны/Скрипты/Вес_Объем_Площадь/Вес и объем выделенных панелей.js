...
function CalcPanel(panel) {
    ...
    VPan = VPan + V;
    den = 0;
    if (Material.toUpperCase().indexOf("ДСП") >=0) //В имени материала есть слово ДСП
    {
       den = 800; //Плотность ДСП
    } else
        {
        if (Material.toUpperCase().indexOf("ДВП") >=0) //В имени материала есть слово ДВП
          {
             den = 350; //Плотность ДВП
          }
          else
            if (Material.toUpperCase().indexOf("МДФ") >=0) //В имени материала есть слово МДФ
            {
               den = 900; //Плотность МДФ
            }
              else
                {
                    if (Material.toUpperCase().indexOf("ЗЕРКАЛО") >=0) //В имени материала есть слово Зеркало
                    {
                       den = 2500; //Плотность зеркала
                    }
                }
        }

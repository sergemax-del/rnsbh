Distance = 0.4; //Расстояние от торца панели с кромкой до другой панели, которая закрывает этот торец
DelButtCount = 0;
...
//            panel.Butts.Delete(i1);
            DelButtCount++;
            //Устанавливаем переменную в true что есть открытая облицовка
            DelButt = true;
        }
        if (DelButt) break;
    }
    ...
    if (DelButt) {
//        panel.Build();
        panel.Selected = true; //Подсвечиваем панель. Признак того, что есть закрытая облицовка.
    }
})

if (DelButtCount > 0)
    alert('Найдено ' + DelButtCount + ' панелей');
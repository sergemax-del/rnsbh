...
Model.forEachPanel(function (panel) {
        ...
        for (var i = 0; i < panel.Butts.Count; i++) {
        var sign = panel.Butts.Butts[i].Allowance;
        if ((sign > 0) && (AllowanceButts.indexOf(sign) < 0)) {
            AllowanceButts.push(sign);
            }
        }
...
if (SpPanelPlastic.length > 1) TipPanelPlastic.AddItem('Все');
SpPanelPlastic.sort();
...
if (SpUserProperty.length > 1) TipPanelUsrProperty.AddItem('Все');
SpUserProperty.sort();
...
if (namesButtMaterials.length > 1) TipMaterialKromki.AddItem('Все');
...
TipKromki.AddItem('');
if (namesButts.length > 1) TipKromki.AddItem('Все');
namesButts.sort();
for (var i = 0; i < namesButts.length; i++) {
...
if (ThicknessButts.length > 1)  ThicknessKromki.AddItem('Все');
ThicknessButts.sort();
...
///////////////////////////////////
//Выделение прифуговки кромок
function SelectAllowanceButts(c) {
    var value = c.Value;
    if (value == '') return;
    ItemAnd = [];
    for (var i = 0; i < Item.length; i++) {
        panel = Item[i];
        for (var i1 = 0; i1 < panel.Butts.Count; i1++) {
            var sign = panel.Butts.Butts[i1].Allowance;
            if ((value == sign) || (value == 'Все')) {
                if (UslAnd) {
                    if (ItemAnd.indexOf(panel) < 0) ItemAnd.push(panel)
                } else {
...
//Создание выпадающего списка и добавление туда прифуговки кромок
AllowanceKromki = GroupButt.NewCombo('Прифуговка кромки', '');
AllowanceKromki.AddItem('');
if (AllowanceButts.length > 1) AllowanceKromki.AddItem('Все');
AllowanceButts.sort();
for (var i = 0; i < AllowanceButts.length; i++) {
    AllowanceKromki.AddItem(AllowanceButts[i]);
}
///////////////////////////////////
...
TipNameFurn.AddItem('');
if (namesFurn.length > 1) TipNameFurn.AddItem('Все');
namesFurn.sort();
...
TipNameHole.AddItem('');
if (namesHole.length > 1) TipNameHole.AddItem('Все');
namesHole.sort();
...
TipCuts.AddItem('');
if (namesCuts.length > 1) TipCuts.AddItem('Все');
namesCuts.sort();
...
function SelectMask() {
    ...
    if (GroupButt.Expanded) {
        ...
        SelectAllowanceButts(AllowanceKromki);
    };

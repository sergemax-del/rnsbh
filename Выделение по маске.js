...
AddSelect = Prop.NewCombo('Добавлять к выделенным',  'Да\nНет');
switch (AddSelect.ItemIndex)
     {
       case 0: { AddSel = true
                 break;
               }
...
SelectModelLimitsBtn = Prop.NewButton('Линии стыка');

function InToList(obj) {
    for (var i = 0; i < obj.Count; i++) {
        objChild = obj.Objects[i];
        if (objChild.Name == 'Линия стыка') objChild.Selected = true;  //Можно проверить на имя объекта
        if (objChild instanceof TFurnBlock) InToList(objChild); //Если это блок, заходим в блок
    }
}
...
SelectModelLimitsBtn.OnClick = function() {
    for (var i = 0; i < Model.Count; i++) {
        obj = Model.Objects[i];
        if (obj.Name == 'Линия стыка') obj.Selected = true;  //Можно проверить на имя объекта
        if (obj instanceof TFurnBlock) InToList(obj); //Если это блок, заходим в блок
    }
};

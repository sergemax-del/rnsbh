...
if (Object.keys(Res.Panels).length) {
    ...
    value = (Res.Panels[key] / 1000000).toFixed(2)
    cena = 0;
                        //что ищем в названии     //сколько стоит
    (key.toString().search('ЛДСП')>-1) && (cena = 1500);
    (key.toString().search('МДФ')>-1) && (cena = 2000);
   (key.toString().search('ХДФ')>-1) && (cena = 500);      //строку копируем бесконечное количество раз

    price = ((Res.Panels[key] / 1000000).toFixed(2))*cena
        AddString('Panels', key, value,price )
    };
}
...
if (Object.keys(Res.Edges).length) {
    ...
    for (var key in Res.Edges) {
        cena = 0 ;
                        //что ищем в названии     //сколько стоит
        (key.toString().search('1/19')>-1) && (cena = 20);

        price = (Res.Edges[key] / 1000).toFixed(2)*cena;
        AddString('Edges', key, (Res.Edges[key] / 1000).toFixed(2),price)
    };
}
...
function AddString(type, key, value, price) {
    if (AddCount)

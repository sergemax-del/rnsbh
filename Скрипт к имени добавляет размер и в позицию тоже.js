...
function AdditionalInfo(obj){
    var result = '';
    if (addSize.Value){
        result += ' ' + GetSize(obj.GSize.x) + '*' + GetSize(obj.GSize.y) + '*' + GetSize(obj.GSize.z);
    }
    return result;
}
...
function NameToPos(obj) {
    obj.ArtPos = obj.Name + AdditionalInfo(obj);
}
// поз в имя
...
function NameAndPos(obj) {
    var value = obj.Name + separator + obj.ArtPos + AdditionalInfo(obj);
    obj.Name = value;
    obj.ArtPos = value;
}

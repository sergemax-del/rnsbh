...
function MakeProperties() {
  ...
  // Добавляем селектор типа паза
  TipPaza = ZadnStenka.NewSelector('Конфигурация паза');
  TipPaza.Items = 'Одна сторона|Г-образный|П-образный (снизу открыто)|П-образный (сверху открыто)';
  TipPaza.ItemIndex = 0; // По умолчанию - одна сторона
...
// Функция построения траектории паза
function BuildPazTrajectory(Traj, PanelType, SizeX, SizeY, Offset, Type) {
  // PanelType: 0=горизонтальная(крышка/дно), 1=вертикальная(боковина)
  // Type: 0=одна сторона, 1=Г-образный, 2=П-образный(снизу открыто), 3=П-образный(сверху открыто)

  switch (Type) {
    case 0: // Одна длинная сторона
      if (PanelType == 0) // Горизонтальная панель
        Traj.AddLine(0, -Offset, SizeX, -Offset);
      else // Вертикальная панель
        Traj.AddLine(Offset, 0, Offset, SizeY);
      break;

    case 1: // Г-образный (длина + одна ширина)
      if (PanelType == 0) {
        Traj.AddLine(0, -Offset, SizeX, -Offset);
        Traj.AddLine(SizeX, -Offset, SizeX, SizeY - Offset);
      } else {
        Traj.AddLine(Offset, 0, Offset, SizeY);
        Traj.AddLine(Offset, SizeY, SizeX - Offset, SizeY);
      }
      break;

    case 2: // П-образный (длина + две ширины, открыто снизу)
      if (PanelType == 0) {
        Traj.AddLine(0, -Offset, SizeX, -Offset);
        Traj.AddLine(SizeX, -Offset, SizeX, SizeY - Offset);
        Traj.AddLine(SizeX, SizeY - Offset, 0, SizeY - Offset);
      } else {
        Traj.AddLine(Offset, 0, Offset, SizeY);
        Traj.AddLine(Offset, SizeY, SizeX - Offset, SizeY);
        Traj.AddLine(SizeX - Offset, SizeY, SizeX - Offset, 0);
      }
      break;

    case 3: // П-образный (две длины + ширина, открыто сверху)
      if (PanelType == 0) {
        Traj.AddLine(0, SizeY - Offset, SizeX, SizeY - Offset);
        Traj.AddLine(SizeX, SizeY - Offset, SizeX, -Offset);
        Traj.AddLine(SizeX, -Offset, 0, -Offset);
      } else {
        Traj.AddLine(SizeX - Offset, 0, SizeX - Offset, SizeY);
        Traj.AddLine(SizeX - Offset, SizeY, Offset, SizeY);
        Traj.AddLine(Offset, SizeY, Offset, 0);
      }
      break;
  }
}
...
function Make() {
  ...
  //Строим паз с выбранной конфигурацией
  Cut = Krishka.AddCut('ЗадСт');
  BuildPazTrajectory(Cut.Trajectory, 0, Dl.Value, Gl.Value, 10, TipPaza.ItemIndex);
  ContPaz = Cut.Contour;
  ...
  BuildPazTrajectory(Cut.Trajectory, 1, Gl.Value, H.Value - Thick, 10, TipPaza.ItemIndex);
  ContPaz = Cut.Contour;
  ContPaz.AddRectangle(0, 0, -ThickZadSt - 0.5, 10);
  LevBok.Build();
  ...
  BuildPazTrajectory(Cut.Trajectory, 0, Dl.Value - Thick, Gl.Value, 10, TipPaza.ItemIndex);
  ContPaz = Cut.Contour;
